...
class PositionalEncoding(nn.Module):
    def __init__(self,d_model:int, seq_len:int, dropout:float) -> None:
        ...
        # create a vector of shape (seq len , 1)
        position = torch.arange(0, seq_len, dtype=self.float).unsqueeze(1)
        ...
        # save pe as non-trainable state that moves with the model device
...
class Transformer(nn.Module):
    ...
    def decode(self, src, encoder_output, src_mask, tgt, tgt_mask):
